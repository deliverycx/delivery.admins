...
import OrganizationListCart from './OrganizationListCart';
...
const OrganizationList = () => {
	...
	const { handlePuckUp, handleHiddenOrg, handleHiddenCity, handleHiddenMobileCity } = useCasePoints.handlers;
...

			{
				...
				sortedCities.map((org: ListOrganization, index: number) => {
					if (org) {
						...
						const CNCityMOB = cn('col-2 btn btn-block', { 'btn-success': org.isHiddenOnMobile });
						return (
							<div key={index} className='card'>
								<div className='card-header'>
									<div className='row'>
										...
										<div className={CNCityMOB}
											onClick={() => handleHiddenMobileCity(
												org._id,
												!org.isHiddenOnMobile)}
										>Скрыть город в мобильном
										</div>
									</div>
								</div>
								{
									org.organizations.map((point: IPoint, i: number) => {
										...
										return (
											<OrganizationListCart key={point.id} point={point} handleHiddenOrg={handleHiddenOrg} />
